...
function formatTransactionId(transactionId2) {
  return `${logger.colors.fg.magenta}#${transactionId2}${logger.colors.reset}`;
}
function formatStep(step, total) {
  return `${logger.colors.bg.black}${logger.colors.fg.yellow}[${step}/${total}]${logger.colors.reset}`;
}
function formatMethod(method) {
  return `${logger.colors.bright}${method}${logger.colors.reset}`;
}
function formatAction(action) {
  return `${logger.colors.dim}(${action})${logger.colors.reset}`;
}
